...
int main(int argc, char **argv)
{
	...
	{
		std::cerr<<"Number of kernel rows and columns has to be odd"<<std::endl;
		return 1;
	}
	...
	if(!kernelfile)
	{
		std::cerr<<"Impossible to read kernel file"<<std::endl;
		return 1;
	}
	...
	Convolutionmp C(inputmatrix,kernelmatrix, rows, columns, krows, kcolumns);
	C.Compute();
