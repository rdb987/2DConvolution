...
ConvolutionMPI::ConvolutionMPI(std::vector<int> input_matrix, std::vector<int> kernel_matrix, int rows, int columns, int krows, int kcolumns)
...
void ConvolutionMPI::Reset(){
	output.resize(rows*columns,0);
}

//Serial implementation
void ConvolutionMPI::Compute(){
...
void ConvolutionMPI::ComputeParallel(std::ofstream &f){
	int rank;
	...
	if(p==1 || p>rows)
		...
		//Input matrix scattering
		std::vector<int> prows(p); // number of rows per process
		std::vector<int> firstrow(p); // First row index per process
		int splitr;
		int extrar;
		splitr = rows / p;
		extrar = rows % p;
		...
		// Exchange of rows between processes

		if(half_kernel!=0  ) // Not needed for bigger kernels
		{
...
void ConvolutionMPI::Print_to_File(std::ofstream &f)
{
	if(!f)
	{
